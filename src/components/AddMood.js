...
export default class AddMood extends React.Component {
    ...
    handleSubmitMood = e => {
        e.preventDefault()
        const { mood, moodNote, category, hours_slept } = e.target
    
        
        MoodApiService.postMood(moodNote.value, Number(mood.value), category.value, hours_slept.value)
        ...
        return (
            <section className="add-mood">
                ...
                <form className="add-mood-form" onSubmit={this.handleSubmitMood}>
                    ...
                    <select name="hours_slept" required>
